...
(function(factory) {
    ...
    var OrgChart = function(elem, opts) {
        ...
        this.defaultOptions = {
            'nodeTitle': 'name',
            'nodeId': 'id',
            'nodeFamilyId': 'familyId',
            'nodeUserParentId': 'userParentId',
    ...
    OrgChart.prototype = {
        ...
        hEdgeClickHandler: function(event) {
            ...
            if (siblingsState.exist) {
                ...
                if (this.startLoading($hEdge)) {
                    ...
                    var familyId = $node[0].familyId;
                    var userParentId = $node[0].userParentId;
                    var generationId = $node[0].generationId;
        ...
        createNode: function(data) {
            ...
            var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data[opts.nodeFamilyId] ? ' familyId="' + data[opts.nodeFamilyId] + '"' : '') + (data[opts.nodeUserParentId] ? ' userparentId="' + data[opts.nodeUserParentId] + '"' : '') + (data[opts.nodeGenerationId] ? ' generationId="' + data[opts.nodeGenerationId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
                .addClass('node ' + (data.className || '') + (level > opts.visibleLevel ? ' slide-up' : ''));
            if (opts.nodeTemplate) {
                $nodeDiv.append(opts.nodeTemplate(data));
            } else {
                $nodeDiv.append('<div class="title red-tooltip" data-container="body" data-placement="bottom" title="' + data[opts.nodeTitle] + '" data-title="' + data[opts.nodeTitle] + '">' + data[opts.nodeTitle] + '</div>')
